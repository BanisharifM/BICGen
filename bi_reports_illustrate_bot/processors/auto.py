...
def report_list(bot: TelegramBot, update: Update, state: TelegramState):
    ...
    try:
        ...
        if msg in ["next", "prev"]:
            ...
            bot.editMessageText(text=get_reports_list(state), chat_id=chat_id, message_id=msg_id,
                                reply_markup=kb, parse_mode="MarkdownV2", disable_web_page_preview=True)
        else:
    ...
    try:
        ...
        if msg == "finish":
            ...
            state_obj.setdefault("states", []).append(
                next_state_name + "_" + state_obj["query"])
        ...
        if msg == "save":
            ...
            if cur_type == 'minMax':
                ...
                if msg in dv.get_column_choices(cur_filter):
                    choices: list = state_obj['cur_filter_config'].get(
                        'choices', [])
                    if msg in choices:
                        choices.remove(msg)
                    else:
    ...
    try:
        ...
        if msg in query_obj['charts']:
            ...
            if rel_path:
                ...
                report_name = query_obj['target'] + ' by ' + \
                    ' and '.join(query_obj['params']) + '_' + msg
                r = Report.objects.create(name=report_name, owner=state.telegram_user,
                                          fig=rel_path, params=query_obj['params'], target=query_obj['target'])
                bot.sendDocument(chat_id, document=open(
                    r.fig.path, "rb"), upload=True)
            else:
                bot.sendMessage(chat_id, MessageText.UEX.value)
    except Exception as e:
        print(str(e))
